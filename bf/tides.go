	...
	for _, feature := range features {
		if feature.PropertyFloat("CurrentTide") != math.NaN() {
			if currTideIn = toTideIn(feature); currTideIn == nil {
				log.Print(pzsvc.AddRef(
					fmt.Errorf(`Could not get tide information from feature %v because required elements did not exist.`, feature.ID)).Error())
				continue
			}
...
func updateSceneTide(scene *geojson.Feature, inpObj tideOut) {
	...
	if err := catalog.SaveFeatureProperties(scene.ID, properties); err != nil {
		log.Print(pzsvc.AddRef(fmt.Errorf("Failed to update feature %v with tide information: %v", scene.ID, err.Error())).Error())
	}
}
